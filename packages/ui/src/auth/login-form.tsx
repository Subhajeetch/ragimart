...
type AuthClient = {
  signIn: {
    social: (opts: { provider: string; callbackURL: string }) => Promise<any>;
...
export default function LoginPage( { authClient, appUrl, onSuccess }: Props) {
 ...
 async function handleEmail(e: React.FormEvent) {
  ...
  return (
    <main className="min-h-screen w-full bg-neutral-950 flex items-center justify-center p-6">
      <div className="w-12xl bg-neutral-900 border border-neutral-800 rounded-2xl p-8 shadow-2xl">
        ...
        {tab === "options" && (
          <div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                onClick={handleApple}
                className="flex items-center justify-center gap-3 w-full px-4 py-3 rounded-xl bg-neutral-100 text-neutral-900 text-sm font-medium hover:bg-white transition-colors cursor-pointer"
              >
                <AppleIcon />
                Continue with Apple
              </button>
